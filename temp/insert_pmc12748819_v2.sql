...
-- Limpa registro órfão da tentativa anterior
DELETE FROM questions WHERE question_id = 'a5b77a90-d846-4582-8a2a-931aabca385d';

-- Inserta questão base
INSERT INTO questions (question_id, canonical_code, status, source)
VALUES (gen_random_uuid(), 'QFE001', 'published', 'PMC12748819')
RETURNING question_id INTO _qid;

-- Inserta versão da questão
...
RETURNING question_version_id INTO _vid;

-- Inserta choices
INSERT INTO question_choices (choice_id, question_version_id, label, choice_text, is_correct, explanation)
VALUES
  (gen_random_uuid(), _vid, 'A', 'Leptin-mediated JAK/STAT signaling pathway', FALSE, 'Leptin is elevated in obesity but primarily regulates appetite; its JAK/STAT signaling is not the direct mediator of acute myocardial injury.'),
  (gen_random_uuid(), _vid, 'B', 'Adiponectin-mediated AMPK cardioprotective pathway', FALSE, 'Adiponectin is cardioprotective and typically downregulated in obesity; its AMPK pathway would be suppressed, not upregulated.'),
  (gen_random_uuid(), _vid, 'C', 'FTO (fat mass and obesity-associated) gene demethylation', FALSE, 'FTO gene variants are associated with obesity risk via epigenetic mechanisms but do not mediate acute myocardial injury.'),
  ...
  (gen_random_uuid(), _vid, 'E', 'PPAR-Σ nuclear receptor downregulation', FALSE, 'PPAR-Σ is a therapeutic target (thiazolidinediones) but its downregulation is not the primary injury mediator in the obesity-MI axis.');

COMMIT;

-- Validação
SELECT 'Questão: ' || q.question_id,
       'Versão: ' || v.version,
       'Choices: ' || COUNT(qc.choice_id)
FROM questions q
LEFT JOIN question_versions v ON v.question_id = q.question_id
LEFT JOIN question_choices qc ON qc.question_version_id = v.question_version_id
